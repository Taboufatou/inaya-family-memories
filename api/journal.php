<?php
require_once 'config.php';

$method = $_SERVER['REQUEST_METHOD'];
$input = json_decode(file_get_contents('php://input'), true);

// Validation du token
$headers = getallheaders();
$token = $headers['Authorization'] ?? '';
if (!validateToken($token)) {
    http_response_code(401);
    echo json_encode(['error' => 'Token invalide']);
    exit;
}

$user = getUserFromToken($token);

if ($method === 'POST') {
    $action = $input['action'] ?? '';
    
    switch ($action) {
        case 'get':
            try {
                $stmt = $pdo->prepare("SELECT * FROM journal_entries ORDER BY entry_date DESC");
                $stmt->execute();
                $entries = $stmt->fetchAll();
                echo json_encode(['success' => true, 'entries' => $entries]);
            } catch (PDOException $e) {
                http_response_code(500);
                echo json_encode(['error' => 'Erreur lors de la récupération']);
            }
            break;

        case 'add':
            if ($user['user_type'] !== 'admin' && $user['user_type'] !== 'papa' && $user['user_type'] !== 'maman') {
                http_response_code(403);
                echo json_encode(['error' => 'Non autorisé']);
                exit;
            }

            $titre = $input['titre'] ?? '';
            $contenu = $input['contenu'] ?? '';
            $date = $input['date'] ?? date('Y-m-d');
            $humeur = $input['humeur'] ?? '';
            $poids = $input['poids'] ?? '';
            $taille = $input['taille'] ?? '';

            if (empty($titre) || empty($contenu)) {
                http_response_code(400);
                echo json_encode(['error' => 'Titre et contenu requis']);
                exit;
            }

            try {
                $stmt = $pdo->prepare("INSERT INTO journal_entries (titre, contenu, entry_date, humeur, poids, taille, author, date_created) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())");
                $stmt->execute([$titre, $contenu, $date, $humeur, $poids, $taille, $user['user_type']]);
                
                echo json_encode(['success' => true, 'id' => $pdo->lastInsertId()]);
            } catch (PDOException $e) {
                http_response_code(500);
                echo json_encode(['error' => 'Erreur lors de la création']);
            }
            break;

        case 'update':
            $id = $input['id'] ?? '';
            if (empty($id)) {
                http_response_code(400);
                echo json_encode(['error' => 'ID requis']);
                exit;
            }

            // Vérifier que l'utilisateur peut modifier cette entrée
            $stmt = $pdo->prepare("SELECT author FROM journal_entries WHERE id = ?");
            $stmt->execute([$id]);
            $entry = $stmt->fetch();

            if (!$entry || ($user['user_type'] !== 'admin' && $entry['author'] !== $user['user_type'])) {
                http_response_code(403);
                echo json_encode(['error' => 'Non autorisé']);
                exit;
            }

            $titre = $input['titre'] ?? '';
            $contenu = $input['contenu'] ?? '';
            $date = $input['date'] ?? '';
            $humeur = $input['humeur'] ?? '';
            $poids = $input['poids'] ?? '';
            $taille = $input['taille'] ?? '';

            try {
                $stmt = $pdo->prepare("UPDATE journal_entries SET titre = ?, contenu = ?, entry_date = ?, humeur = ?, poids = ?, taille = ? WHERE id = ?");
                $stmt->execute([$titre, $contenu, $date, $humeur, $poids, $taille, $id]);
                
                echo json_encode(['success' => true]);
            } catch (PDOException $e) {
                http_response_code(500);
                echo json_encode(['error' => 'Erreur lors de la modification']);
            }
            break;

        case 'delete':
            $id = $input['id'] ?? '';
            if (empty($id)) {
                http_response_code(400);
                echo json_encode(['error' => 'ID requis']);
                exit;
            }

            // Vérifier que l'utilisateur peut supprimer cette entrée
            $stmt = $pdo->prepare("SELECT author FROM journal_entries WHERE id = ?");
            $stmt->execute([$id]);
            $entry = $stmt->fetch();

            if (!$entry || ($user['user_type'] !== 'admin' && $entry['author'] !== $user['user_type'])) {
                http_response_code(403);
                echo json_encode(['error' => 'Non autorisé']);
                exit;
            }

            try {
                $stmt = $pdo->prepare("DELETE FROM journal_entries WHERE id = ?");
                $stmt->execute([$id]);
                
                echo json_encode(['success' => true]);
            } catch (PDOException $e) {
                http_response_code(500);
                echo json_encode(['error' => 'Erreur lors de la suppression']);
            }
            break;
            
        default:
            http_response_code(400);
            echo json_encode(['error' => 'Action non reconnue']);
    }
}
?>