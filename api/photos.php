...
switch ($method) {
    ...
    case 'POST':
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO photos (title, description, url, category, author, date_created) VALUES (?, ?, ?, ?, ?, NOW())");
    ...
    case 'PUT':
        ...
        if (!$photo || ($user['type'] !== 'admin' && $photo['author'] !== $user['type'])) {
            http_response_code(403);
            echo json_encode(['error' => 'Non autorisé']);
            exit;
        }
        ...
        try {
            $stmt = $pdo->prepare("UPDATE photos SET title = ?, description = ?, url = ?, category = ? WHERE id = ?");
